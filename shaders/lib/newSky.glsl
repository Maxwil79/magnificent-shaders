...
float sunIlluminance = 1e4;
float moonIlluminance = sunIlluminance * 4e-8; //Not really physically based.

#if FastSky == 0
float exposure = 5.0e-14;
#elif FastSky == 1
float exposure = 15e-14;
#endif
...
void atmosphereResult (inout vec4 color, in vec2 coord, in vec3 view, in int samples1, in int samples2, in vec3 sunView) 
{
	...
	vec3 xyz = vec3(0.0);
    {
        vec4 wl = vec4(625.0, 540.0, 450.0, 0.0);
