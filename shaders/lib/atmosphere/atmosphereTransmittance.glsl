...
vec3 atmosphereTransmittance2(vec3 sunVector, vec3 upVector, vec3 moonVector) {
    const int steps = skyQuality_I;
    ...
    opticalDepth *= stepSize;
    vec3 transmittance = exp(rayleighTransmittanceCoefficient * opticalDepth.x + mieTransmittanceCoefficient * opticalDepth.y) * sunColor * 12e-3;
