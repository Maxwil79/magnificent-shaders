...
vec2 iResolution = vec2(viewHeight, viewWidth);
...
vec3 calculateMoon(
    ...
    // PLANET
    	float r = 1e3; // radius
    	float z_in = sqrt(r*r - moonVector.x*moonVector.x - moonVector.y*moonVector.y);
    	float z_out = sqrt(-r*r + moonVector.x*moonVector.x + moonVector.y*moonVector.y);
    
    // NORMALS
    	vec3 norm = normalize(vec3(moonVector.x, moonVector.y, z_in)); // normals from sphere
    	vec3 norm_out = normalize(vec3(moonVector.x, moonVector.y, z_out)); // normals from outside sphere
