...
vec3 lowlightDesaturate(vec3 color) {
    ...
    color = mix(color, vec3(desaturated) * vec3(0.6, 0.7, 1.20), exp2(-150.0 * desaturated));
...
void main() {
    ...
    float averageBrightness = dot(textureLod(colortex0, vec2(0.5), log2(max(viewWidth, viewHeight))).rgb, vec3(3.0 / 0.15));
    float exposure = clamp(3.0 / averageBrightness, 2.5e-2, 0.33e1);
